...
@PartitionerPlugin(
   id="BucketPartitioner",
   name="Bucket",
   description="Quotient of division"
   )
public class BucketPartitioner extends BasePartitioner {

	private String fieldName; 
	protected int partitionColumnIndex = -1;
	protected int groupIndex = 0;
	
	public BucketPartitioner(  ) {
		super( );
	}

	public Partitioner getInstance() 
	{
		Partitioner partitioner = new BucketPartitioner();
		partitioner.setId( getId() );
		partitioner.setDescription( getDescription() );
		return partitioner;
	}
	
	public BucketPartitioner clone()
	{
		BucketPartitioner bucketPartitioner = (BucketPartitioner) super.clone();
		bucketPartitioner.fieldName = fieldName;
	...
	{
		return "org.pentaho.di.ui.trans.dialog.BucketPartitionerDialog";
	}
		...
		public int getPartition(RowMetaInterface rowMeta, Object[] row ) throws KettleException
		{
	        ...
	        int targetLocation = (int)((Math.abs(value) / nrPartitions) % nrPartitions);

			return targetLocation;
		}

		public String getDescription() {
			String description = "Bucket partitioner";
